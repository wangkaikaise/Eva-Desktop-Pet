...
- CPU/GPU 占用率、CPU/GPU 温度卡片，支持上、下、左、右、字体、文字颜色和完全透明背景。
...
CPU 温度需要可选的 LibreHardwareMonitor + PawnIO 扩展。程序主体始终以普通用户权限运行；
只有用户主动选择“启用 CPU 温度”时，温度助手才请求 UAC。读取失败会明确显示“不可用”，
不会猜测温度。
